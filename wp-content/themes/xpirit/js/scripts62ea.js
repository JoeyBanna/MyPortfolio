...
jQuery(".nav-mobile .menu-item-has-children").click(toggle_mobile_navigation_dropdown);
...
customElements.define('a11y-user', class extends HTMLElement {
  constructor() {
    super();
...
{ /* Smooth Scroll widget
  Objectives:
  - use DOM builtins
  - focus the anchor target to enable normal keyboard navigation
  */
...
// SLIDERS
// Logo slider
jQuery('.has-logo-slider').each(function () {
	var $logoSlider = jQuery(this).find('.logo-slider').flickity({
		pageDots: false,
		prevNextButtons: false,
		cellSelector: '.logo-slider-item',
		wrapAround: true,
		imagesLoaded: true,
		cellAlign: 'left'
	});
...
jQuery(window).on('scroll', function() {
	...
	if (pos >= 50) {
		jQuery(".main-navigation").addClass("is-gone");
	} else {
		jQuery(".main-navigation").removeClass("is-gone");
	}  
});

jQuery('.cases-slider-main').flickity({
  pageDots: false,
  initialIndex: 1,
  bgLazyLoad: 2,
  prevNextButtons: false
...
jQuery('.cases-slider-nav').flickity({
...
//Team Slider
var $teamSlider = jQuery('.team-slider').flickity({
  pageDots: false,
  prevNextButtons: false,
...
jQuery('.team-nav .btn-left').on('click', function () {
  $teamSlider.flickity('previous');
});
jQuery('.team-nav .btn-right').on('click', function () {
  $teamSlider.flickity('next');
});

//Newsletter toggle
// function toggle_newsletter() {
//   jQuery('.newsletter-form').toggleClass('is-active');
//   console.log('click!');
// }
...
//   jQuery('.newsletter-form form').on('click', function () {
//     console.log('click!');
//   });
// });
